        ...
        <div className="classic-shell">
            <div className="relative flex min-h-screen items-stretch">
                <aside className={`classic-sidebar hidden lg:flex self-stretch sticky top-0 h-dvh min-h-dvh shrink-0 ${collapsed ? 'w-[82px]' : 'w-[258px]'} transition-all duration-500 ease-out`}>
                    <div className="flex h-[84px] items-center gap-3 border-b border-white/10 px-4">
                                    ...
                                    <div key={group.key} className="mb-1">
                                        {!collapsed && (
                                            <button
                                                ...
                                                className="flex w-full items-center justify-between rounded-lg px-2 py-2 text-left text-[11px] font-bold uppercase tracking-[0.28em] text-slate-400 hover:bg-white/5"
                                            >
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            <span className="flex items-center gap-2">
                                                                {item.accent ? (
                                                                    <span className="rounded-full bg-cyan-500/20 px-2 py-1 text-[10px] font-bold tracking-[0.16em] text-cyan-100">
                                                                        RELASI
                                                                    </span>
                                                                ) : item.badge ? (
                    ...
                    <div className="border-t border-white/10 bg-gradient-to-t from-slate-950/25 to-transparent p-2 pt-3">
                        <Link
                            href={route('logout')}
                            method="post"
                            as="button"
                ...
                {collapsed && floatingMenu && (
                    ...
                    >
                        <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">Menu</p>
                        <p className="mt-1 text-sm font-bold text-slate-100">{floatingMenu.item.label}</p>
                        <div className="mt-3">
                ...
                <div className="flex min-w-0 flex-1 flex-col">
                    <nav className="classic-topbar flex items-center justify-between px-4 lg:px-6">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-base font-extrabold text-slate-800 sm:text-lg">Sistem Informasi Akademik</span>
                        </div>
                            ...
                            <Dropdown>
                                ...
                                <Dropdown.Content align="right" width="80">
                                    <div className="flex items-center justify-between gap-3 px-3 py-2">
                                        <div>
                                            <p className="text-xs font-bold text-slate-700">Notifikasi Terbaru</p>
                                            <p className="text-[11px] text-slate-500">{unreadCount} belum dibaca</p>
                                        </div>
                                        {unreadCount > 0 && (
                                            <Link href={route('notifications.index')} className="rounded-full bg-slate-900 px-3 py-1 text-[11px] font-bold text-white">
                                                Buka Inbox
                                            </Link>
                                        )}
                                    </div>
                                    {latestUnread.length ? latestUnread.map((n) => (
                                        <div key={n.id} className="border-t border-slate-100 px-3 py-2">
                                            <p className="text-xs font-semibold text-slate-700">{n.data?.title || 'Notifikasi'}</p>
                                            <p className="mt-1 text-[11px] text-slate-500">{n.data?.message || '-'}</p>
                                            <div className="mt-2 flex items-center gap-2">
                                                <Dropdown.Link href={route('notifications.read', n.id)} method="patch" as="button">Tandai Dibaca</Dropdown.Link>
                                                <Link href={hrefFor(resolveNotificationAction(n))} className="rounded-md px-2 py-1 text-xs font-semibold text-sky-700 hover:bg-sky-50">
                                    ...
                                    <div className="border-t border-slate-100 px-3 py-2">
                                        <Link href={route('notifications.index')} className="text-xs font-bold text-sky-700">Lihat semua notifikasi</Link>
                                    </div>
                                </Dropdown.Content>
                            </Dropdown>

                            <img src="/logostai.png" alt="Logo" className="h-8 w-8 rounded-full border border-slate-200 bg-white p-1 object-contain" />
                            <span className="hidden text-xs font-bold text-slate-800 md:block">{user.name}</span>
                            <Dropdown>
                                <Dropdown.Trigger>
                        ...
                        <div className="fixed inset-0 z-40 lg:hidden">
                            ...
                            <div className="mobile-sidebar-panel absolute left-2 top-[70px] bottom-2 w-[88vw] max-w-[280px] overflow-y-auto rounded-2xl p-2 shadow-2xl">
                                <div className="flex items-center justify-between border-b border-white/10 px-2 pb-2">
                                    <p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-300">Menu</p>
                                    <button className="top-btn h-7 w-7 text-xs" onClick={() => setShowMobileMenu(false)} aria-label="Close menu">
                                        ...
                                        <div className="mt-2">
                                            {groupedMenu.map((group) => (
                                                <div key={group.key} className="mb-1">
                                                    <p className="px-2 py-1 text-[11px] font-bold uppercase tracking-wide text-slate-300">{group.group}</p>
                                                    {group.items.map((item) => (
