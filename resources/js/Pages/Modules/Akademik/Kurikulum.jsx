    ...
    return (
        <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
            ...
            <div>
                ...
                <p className="text-sm font-semibold text-slate-800">Harap tunggu, data kurikulum sedang disimpan.</p>
            </div>
        </div>
    );
}
...
function RowActions({ onEdit, onDelete, kode }) {
    return (
        <div className="flex items-center gap-2">
            <IconButton variant="neutral" label={`Edit kurikulum ${kode}`} onClick={onEdit}>
                <ActionIcon name="edit" />
            </IconButton>
            <IconButton variant="danger" label={`Hapus kurikulum ${kode}`} onClick={onDelete}>
...
export default function Page({ auth, tabs = [], filters = null, prodis = [], kurikulumList = { data: [], meta: null, links: [] } }) {
    ...
    const data = kurikulumList.data || [];
    const meta = kurikulumList.meta || {};
    const links = kurikulumList.links || [];

    const form = useForm({ prodi_id: '', nama: '', kode: '', tahun_berlaku: new Date().getFullYear(), is_active: false });
    const edit = useForm({ prodi_id: '', nama: '', kode: '', tahun_berlaku: new Date().getFullYear(), is_active: false });
    ...
    const applyFilters = (pageUrl = null) => {
        ...
        router.get(route('akademik.kurikulum.index'), { search, per_page: perPage }, { preserveScroll: true, preserveState: true });
    };

    const resetFilters = () => {
        setSearch('');
        setPerPage('10');
        router.get(route('akademik.kurikulum.index'), {}, { preserveScroll: true, preserveState: true });
    };

    const submit = (e) => {
        e.preventDefault();
        form.post(route('akademik.kurikulum.store'), {
            preserveScroll: true,
            onSuccess: () => form.reset('prodi_id', 'nama', 'kode', 'tahun_berlaku', 'is_active'),
        });
    };

    const handleImport = (e) => {
        e.preventDefault();
        importForm.post(route('akademik.kurikulum.import'), {
            preserveScroll: true,
            forceFormData: true,
    ...
    const removeItem = () => {
        deleteForm.delete(route('akademik.kurikulum.destroy', itemToDelete.id), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    return (
        <PageShell auth={auth} title="Kurikulum" tabs={tabs} showFlash={false} heroVariant="corner" layoutHeader={false}>
            <ConfirmationModal
                show={confirmingDeletion}
                onClose={() => setConfirmingDeletion(false)}
                onConfirm={removeItem}
                title="Hapus Kurikulum"
                message={`Apakah Anda yakin ingin menghapus kurikulum ${itemToDelete?.nama}? Data yang sudah dihapus tidak dapat dikembalikan.`}
                ...
                <div className="space-y-4">
                    <form onSubmit={submit} className="panel space-y-3 p-4">
                        <div>
                            ...
                            <h3 className="mt-1 text-sm font-bold text-slate-900">Tambah Kurikulum</h3>
                        </div>
                        <div>
                        ...
                        <div>
                            <label className="mb-1 block text-xs font-bold text-slate-700">Kode Kurikulum</label>
                        ...
                        <div>
                            <label className="mb-1 block text-xs font-bold text-slate-700">Nama Kurikulum</label>
                            <input className="form-input" placeholder="Nama Kurikulum" value={form.data.nama} onChange={(e) => form.setData('nama', e.target.value)} />
                            <FieldError message={form.errors.nama} />
                        </div>
                        <div>
                            <label className="mb-1 block text-xs font-bold text-slate-700">Tahun Berlaku</label>
                            <input type="number" className="form-input" placeholder="Tahun Berlaku" value={form.data.tahun_berlaku} onChange={(e) => form.setData('tahun_berlaku', e.target.value)} />
                            <FieldError message={form.errors.tahun_berlaku} />
                        </div>
                    ...
                    <section className="panel relative space-y-3 p-4">
                        ...
                        <a href={route('akademik.kurikulum.template')} className="btn-outline flex w-full items-center justify-center">
                            Download Template
                        </a>
                        <form onSubmit={handleImport} className="space-y-3">
                            ...
                            <button className="btn-primary w-full" type="submit" disabled={importForm.processing || !importForm.data.file}>
                                {importForm.processing ? 'Mengimpor...' : 'Import Kurikulum'}
                            </button>
                        </form>
                        <p className="text-xs text-slate-500">Kolom yang dibaca: prodi_kode, kode, nama, tahun_berlaku, is_active.</p>
                ...
                <section className="panel p-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                            ...
                            <h3 className="mt-1 text-sm font-bold text-slate-900">Kurikulum Terdaftar</h3>
                        </div>
                        <div className="flex flex-wrap gap-2">
                            <a href={route('akademik.kurikulum.export', { search, per_page: perPage })} className="btn-outline">
                                Export Excel
                            </a>
                    ...
                    <div className="mt-4 grid gap-2 lg:grid-cols-[minmax(0,1fr)_140px_auto_auto]">
                        <input
                            ...
                            placeholder="Cari kode, nama, tahun, atau prodi"
                            value={search}
                    ...
                    <div className="mt-4 space-y-3 md:hidden">
                        {data.length ? data.map((item) => (
                            <Fragment key={item.id}>
                                <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                                    ...
                                    <p className="mt-2 text-sm leading-relaxed text-slate-600">{item.prodi?.nama || '-'} | Tahun {item.tahun_berlaku}</p>
                                    <div className="mt-4">
                                ...
                                {editingId === item.id && (
                                    <form
                                        onSubmit={(e) => {
                                            e.preventDefault();
                                            edit.put(route('akademik.kurikulum.update', item.id), {
                                                preserveScroll: true,
                                    ...
                                    >
                                        ...
                                        <div>
                                            <input type="number" className="form-input" value={edit.data.tahun_berlaku} onChange={(e) => edit.setData('tahun_berlaku', e.target.value)} />
                                            <FieldError message={edit.errors.tahun_berlaku} />
                                        </div>
                        ...
                        )) : <EmptyState title="Belum ada kurikulum" description="Kurikulum mengatur daftar mata kuliah yang berlaku pada periode tertentu." />}
                    </div>
                                ...
                                {data.length ? data.map((item) => (
                                    <Fragment key={item.id}>
                                        <tr className="border-t border-slate-100 align-top">
                                            ...
                                            <td className="px-3 py-3 text-slate-600">{item.tahun_berlaku}</td>
