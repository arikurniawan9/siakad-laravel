        ...
        <div className="mt-4 flex flex-col gap-3 border-t border-slate-200 pt-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
                {links.map((link, index) => (
                    <button key={`${link.label}-${index}`} type="button" disabled={!link.url} onClick={() => link.url && router.get(link.url, {}, { preserveScroll: true, preserveState: true })} className={`rounded-xl px-3 py-2 text-xs font-semibold ${link.active ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 disabled:opacity-50'}`}>{link.label}</button>
                ))}
            </div>
        </div>
    );
    ...
    return (
        <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
            ...
            <div>
                ...
                <p className="text-sm font-semibold text-slate-800">Harap tunggu, data ruangan sedang disimpan.</p>
            </div>
        </div>
    );
}
...
function RowActions({ onEdit, onDelete, kode }) {
    return (
        <div className="flex items-center gap-2">
            <IconButton variant="neutral" label={`Edit ruangan ${kode}`} onClick={onEdit}>
                <ActionIcon name="edit" />
            </IconButton>
            <IconButton variant="danger" label={`Hapus ruangan ${kode}`} onClick={onDelete}>
...
export default function Page({ auth, tabs = [], filters = null, ruangans = { data: [], meta: null, links: [] } }) {
    ...
    const data = ruangans.data || [];
    const meta = ruangans.meta || {};
    const links = ruangans.links || [];

    const form = useForm({ kode: '', nama: '', gedung: '', kapasitas: 40, is_active: true });
    const edit = useForm({ kode: '', nama: '', gedung: '', kapasitas: 40, is_active: true });
    ...
    const applyFilters = (pageUrl = null) => { 
        ...
        router.get(route('akademik.ruangan.index'), { search, per_page: perPage }, { preserveScroll: true, preserveState: true }); 
    };

    const resetFilters = () => { 
        setSearch(''); 
        setPerPage('10'); 
        router.get(route('akademik.ruangan.index'), {}, { preserveScroll: true, preserveState: true }); 
    };

    const submit = (e) => { 
        e.preventDefault(); 
        form.post(route('akademik.ruangan.store'), { 
            preserveScroll: true, 
            onSuccess: () => form.reset('kode', 'nama', 'gedung', 'kapasitas', 'is_active') 
        }); 
    };

    const handleImport = (e) => { 
        e.preventDefault(); 
        importForm.post(route('akademik.ruangan.import'), { 
            preserveScroll: true, 
            forceFormData: true, 
    ...
    const removeItem = () => {
        deleteForm.delete(route('akademik.ruangan.destroy', itemToDelete.id), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    return (
        <PageShell auth={auth} title="Ruangan" tabs={tabs} showFlash={false}>
            <ConfirmationModal
                ...
                title="Hapus Ruangan"
                message={`Apakah Anda yakin ingin menghapus ruangan ${itemToDelete?.nama}? Data yang sudah dihapus tidak dapat dikembalikan.`}
            ...
            <div className="grid gap-4 xl:grid-cols-[360px_minmax(0,1fr)]">
                <div className="space-y-4">
                    <form onSubmit={submit} className="panel space-y-3 p-4">
                        ...
                        <button className="btn-primary w-full" type="submit" disabled={form.processing}>Simpan Ruangan</button>
                    </form>
                    ...
                    <section className="panel relative space-y-3 p-4">
                        ...
                        <a href={route('akademik.ruangan.template')} className="btn-outline flex w-full items-center justify-center">Download Template</a>
                        <form onSubmit={handleImport} className="space-y-3">
                            ...
                            <button className="btn-primary w-full" type="submit" disabled={importForm.processing || !importForm.data.file}>
                                {importForm.processing ? 'Mengimpor...' : 'Import Ruangan'}
                            </button>
                        </form>
                        <p className="text-xs text-slate-500">Kolom yang dibaca: kode, nama, gedung, kapasitas, is_active.</p>
                ...
                <section className="panel p-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                            ...
                            <h3 className="mt-1 text-sm font-bold text-slate-900">Ruangan Terdaftar</h3>
                        </div>
                        <div className="flex flex-wrap gap-2">
                            <a href={route('akademik.ruangan.export', { search, per_page: perPage })} className="btn-outline">Export Excel</a>
                    ...
                    <div className="mt-4 grid gap-2 lg:grid-cols-[minmax(0,1fr)_140px_auto_auto]">
                        <input className="form-input" placeholder="Cari kode, nama, atau gedung" value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && applyFilters()} />
                    ...
                    <div className="mt-4 space-y-3 md:hidden">
                        {data.length ? data.map((item) => (
                            <Fragment key={item.id}>
                                <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                                    ...
                                    <p className="mt-2 text-sm leading-relaxed text-slate-600">{item.gedung || '-'} | Kapasitas {item.kapasitas}</p>
                                    <div className="mt-4">
                                        <RowActions onEdit={() => startEdit(item)} onDelete={() => confirmDeletion(item)} kode={item.kode} />
                                    </div>
                                </article>
                                {editingId === item.id && (
                                    <form onSubmit={(e) => { e.preventDefault(); edit.put(route('akademik.ruangan.update', item.id), { preserveScroll: true, onSuccess: () => setEditingId(null) }); }} className="rounded-2xl border border-sky-200 bg-sky-50/60 p-4 space-y-3">
                                        <div>
                                        ...
                                        <div>
                                            <input className="form-input" value={edit.data.gedung} onChange={(e) => edit.setData('gedung', e.target.value)} />
                                            <FieldError message={edit.errors.gedung} />
                                        </div>
                                        <div>
                                            <input type="number" className="form-input" value={edit.data.kapasitas} onChange={(e) => edit.setData('kapasitas', e.target.value)} />
                                            <FieldError message={edit.errors.kapasitas} />
                                        </div>
                        ...
                        )) : <EmptyState title="Belum ada ruangan" description="Tambahkan ruangan untuk mulai menjadwalkan perkuliahan." />}
                    </div>
                        ...
                        <table className="min-w-full text-left text-xs">
                            <thead className="sticky top-0 z-10 bg-slate-50">
                                <tr className="text-slate-500">
                                    ...
                                    <th className="px-3 py-2">Gedung</th>
                                ...
                                {data.length ? data.map((item) => (
                                    <Fragment key={item.id}>
                                        <tr className="border-t border-slate-100 align-top">
                                            ...
                                            <td className="px-3 py-3 text-slate-600">{item.gedung || '-'}</td>
                                            <td className="px-3 py-3 text-slate-600">{item.kapasitas}</td>
                                        ...
                                        {editingId === item.id && (
                                            <tr className="border-t border-slate-100 bg-sky-50/60">
                                                <td colSpan="6" className="px-3 py-4">
                                                    <form onSubmit={(e) => { e.preventDefault(); edit.put(route('akademik.ruangan.update', item.id), { preserveScroll: true, onSuccess: () => setEditingId(null) }); }} className="grid gap-3 lg:grid-cols-2">
                                                        ...
                                                        <div><input className="form-input" value={edit.data.gedung} onChange={(e) => edit.setData('gedung', e.target.value)} /><FieldError message={edit.errors.gedung} /></div>
                                                        <div><input type="number" className="form-input" value={edit.data.kapasitas} onChange={(e) => edit.setData('kapasitas', e.target.value)} /><FieldError message={edit.errors.kapasitas} /></div>
                                    ...
                                    <tr><td colSpan="6" className="px-3 py-6"><EmptyState title="Belum ada ruangan" description="Tambahkan ruangan untuk mulai menjadwalkan perkuliahan." /></td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
